...
<ul class="navbar-nav">
  ...
  <li class="nav-item d-none d-sm-inline-block">
    <a href="class_details.php" class="nav-link">Class</a>
  </li>
  <li class="nav-item d-none d-sm-inline-block">
    <a href="assignment.php" class="nav-link active">Assignment</a>
  </li>
  <li class="nav-item d-none d-sm-inline-block">
            ...
            <div class="card">
              <div class="card-header">
                ...
                <div class="float-right">
                  <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#modal-add-new"><i class="far fa-file-alt"></i> Create Assignment</button>
              </div>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example3" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <th>Title</th>
                    <th>Instruction</th>
                    <th width = "15%">Due Date <br/><b style="font-size:12px">(Month/Day/Year)</b></th>
...
<button class="btn btn-primary btn-sm btn-deactivate" style = "width: 100px;"><i class="fas fa-user-slash"></i>Deactivate</button>
    ...
    $sql_work = "SELECT * FROM student_work ORDER BY id ASC";
    ...
    ?>
        <div class="row">
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Student Assignments</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example3" class="table table-bordered table-striped">
                  ...
                  if (mysqli_num_rows($result_work) > 0) {
                    ...
                    while($row = mysqli_fetch_assoc($result_work)) { ?>
                      <tr id = <?php echo $row["id"]; ?>>
                        ...
                        <td>
                          <button class="btn btn-info btn-sm btn-grade">
                              <i class="fas fa-pencil-alt"></i>
                             Grade
                          </button>
                        </td>
        ...
        <form id = "form_edit">
            <div class="modal fade" id="modal-edit">
              <div class="modal-dialog modal-xl">
                <div class="modal-content">
                  <div class="modal-header">
                    <h4 class="modal-title">Edit New Assignment</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
                  <div class="modal-body">
                    <input type="hidden" id="edit_class_id" value="1">
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-4">
                        <div class="form-group">
                          <label>Due Date</label>
                          <input type="datetime-local" class="form-control" name="edit_due_date" id="edit_due_date">
                        </div>
                      </div>
                    </div>
                  ...
                  <div class="modal-body">
                    ...
                    <div class="row">
                      <div class="col-md-8">
                          <div class="form-group">
                          <label>Title</label>
                          <input type="text" class="form-control" name="edit_grade" id="edit_grade" placeholder="Enter grade">
                        </div>
                        <div class="modal-footer justify-content-between">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
                    <input type="submit" class="btn btn-primary" id = "btnUpdate" value = "Update">
...
$("#form_new").submit(function(event) {
  ...
  var Toast = Swal.mixin({
    ...
    $.ajax({
        type: "POST",
        cache: false,
        url: 'process_faculty_assignment.php',
        //data: form.serialize(),
        data: {title: title, class_id: class_id, due_date: due_date,instruction: instruction,type:'insert'},
        success:function(data){
          var response_data = JSON.parse(data);
          Toast.fire({
          icon: response_data.icon,
...
$("#form_edit").submit(function(event) {
  ...
  var Toast = Swal.mixin({
    ...
    $.ajax({
        ...
        url: 'process_faculty_assignment.php',
        //data: form.serialize(),
        data: {edit_title: edit_title, edit_class_id: edit_class_id, edit_instruction: edit_instruction,edit_due_date: edit_due_date,type:'edit'},
        success:function(data){
          var response_data = JSON.parse(data);
          Toast.fire({
          icon: response_data.icon,
...
$("#form_grade").submit(function(event) {
  event.preventDefault(); //prevent submit
  var form = $(this);
  var edit_class_id = $('#edit_grade').val();
  var Toast = Swal.mixin({
            toast: true,
            position: 'top-end',
            showConfirmButton: false,
    ...
    $.ajax({
        ...
        url: 'process_faculty_assignment.php',
        //data: form.serialize(),
        data: {edit_grade: edit_grade,type:'edit'},
        success:function(data){
          var response_data = JSON.parse(data);
          Toast.fire({
          icon: response_data.icon,
...
$('#example3 tbody').on('click', '.btn-edit', function () {
  ...
  document.getElementById("edit_due_date").value = $(this).closest("tr").children(":eq(3)").attr('data-due_date');
  $('#modal-edit').modal('show');

});


$('#example3 tbody').on('click', '.btn-grade', function () {


document.getElementById("edit_class_id").value = $(this).closest("tr").attr('id');
document.getElementById("edit_grade").value = $(this).closest("tr").children(":eq(4)").text().trim();
...
Swal.fire({
  title: 'Are you sure?',
  text: "This will delete assignment form to this class.",
  icon: 'warning',
  showCancelButton: true,
  confirmButtonColor: '#3085d6',
  cancelButtonColor: '#d33',
  confirmButtonText: 'Yes, Delete classwork assignment!'
}).then((result) => {
  if (result.isConfirmed) {
    $.ajax({
          ...
          if(response_data.icon == 'success')
          {
            ...
            Swal.fire(
              'Deleted!',
              'Classwork assignment has been deleted.',
              'success'
            )
            setTimeout(function(){location.reload();},2000);
          }
